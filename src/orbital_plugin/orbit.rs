...
pub fn orbit_motion_system( mut query: Query<(&mut Transform, &mut Orbit)>, time: Res<Time> ) {
        ...
        query.par_iter_mut()
             .for_each_mut(|(mut transform, mut orbit)| {
                 ...
                 let rotation = Quat::from_rotation_x(orbit.inclination as f32).mul_quat(
                 ...
                 transform.translation = rotation.mul_vec3(position2);
                 
                 //TODO figure out where to draw the orbit path
...
mod tests {
    ...
    fn orbital_math() {
        ...
        assert_eq!(orbit.eccentric_anomaly(), 0.4755689321854411);
        
        //test mean anomaly
        assert_eq!(orbit.mean_anomaly(), 0.42978450014676683);
